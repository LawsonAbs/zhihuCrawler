...
public class HttpClientUtils {
    ...
    public boolean isEnd(String jsonString) {
        return jsonUtils.isEnd(jsonString);
    }

    public boolean isEnd(HttpEntity entity) {
    ...
    public Set<String> getImageUrl(String topicUrl) {
        Set<String> pictureUrl = new HashSet<String>();

        //01.通过url获取entity
        int i = 50;
        int offset = 0;
        while(i > 0){
            HttpEntity entity = getEntity(topicUrl);
            //HttpEntity entity = getBatchEntity(topicUrl,offset);
            try {
                ...
                for (Element e : imgElements) {
                    ...
                    pictureUrl.add(tempUrl);
                   // System.out.println("src:"+pictureUrl);
                }
                System.out.println(pictureUrl.size());
            } catch (IOException e) {
                e.printStackTrace();
            }
            offset +=20;
    ...
    public Set<String> getImageUrlByJson(HttpEntity entity) {
        Set<String> pictureUrl ;
        try {
            //01.将entity转化为字符串形式的content
            String content = EntityUtils.toString(entity, "utf-8");
            jsonUtils.getImageUrl(content);
        } catch (IOException e) {
            e.printStackTrace();
        }
        //02. get pictureUrl
